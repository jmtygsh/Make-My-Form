...
import {
    Copy, Check, Globe, Link as LinkIcon, Loader2, Download,
...
function indexBlocks(payload: unknown) {
    ...
    for (const block of blocks) {
        ...
        labelByBlockId.set(block.id, ('label' in block && block.label) ? block.label : 'Untitled question');
        if ('options' in block) {
...
export default function FormSettingsPage() {
    ...
    const expiryValue = form?.isExpiry
        ? new Date(form.isExpiry).toISOString().slice(0, 16)
        : '';
    ...
    return (
        <div className="flex flex-col gap-10">
            ...
            <section>
                ...
                <p className="mt-1 text-sm text-gray-500">
                    Public forms can be listed and shared widely. Unlisted forms are only reachable by direct link.
                </p>
                <div className="mt-3 flex gap-2">
                    {(['public', 'unlisted'] as const).map((v) => {
                        ...
                        return (
                            ...
                            >
                                {v === 'public' ? <Globe className="h-3.5 w-3.5" /> : <LinkIcon className="h-3.5 w-3.5" />}
                                {v}
                            </button>
                        );
                    })}
